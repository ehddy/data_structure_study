...
class BinaryTree:
    ...
    # Binary Tree 클래스에 주어진 이진트리를 복사(copy)하는 메소드를 추가 
    def mirror_copy(self):
        # 복사본 노드 생성 
        mirror = Node(self.root)

        # 복사할 노드 지정 
        copy_target = Node(self.root)
        
        # 재귀호출을 통해 target의 노드 구조 copy 
        if mirror.right is not None:
            copy_target.left = mirror.right.mirror_copy()
        if mirror.left is not None:
            copy_target.right = mirror.left.mirror_copy()
